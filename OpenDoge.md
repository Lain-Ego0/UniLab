...
### Round 10 (零速贴地步高, 1000 iters) — 静止时 swing 脚球贴地
```yaml
# 代码关键: 最低步高 0.02→0.015（=脚球半径），零速时脚贴地无多余抬腿
# 步骤目标: 0.015 + 0.105 * cmd_mag → [0.015, 0.12]
# target_foot_height: 0.04→0.03（safe_height: 0.02→0.015）
```
**动机**：Round 9 零速时仍要求 2cm 抬腿，不合理 — 静止应脚贴地。
**结果**：best=**90.57**, final=**79.97**。best 略低于 R9 但 final 更高，收敛更稳。零速贴地逻辑正确。
